...
After chunk 7 the deterministic ledger, Monte Carlo, integrated property, bounded reverse solvers
and marginal capital allocation are implemented. Six screens are built; saved scenarios and
attribution remain open. See `handoffs/chunk-7.md` for 208 passing tests and real Chrome evidence,
`handoffs/chunk-6.md` for solver interfaces and `property-model.md` for property boundaries.
`docs/v0.3-requirements-checklist.md` remains the longer package-10 acceptance audit.

| # | MUST requirement | Owning chunks | Status / evidence after chunk 7 |
| --- | --- | --- | --- |
